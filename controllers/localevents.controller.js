...
const LocalEvent = db.LocalEvent;
...
module.exports.createLocalEvent = (req, res, next) => {
...
module.exports.updateInternally = req => {
  return new Promise((resolve, reject) => {
    LocalEvent.findOne({
      ...
      where: { id: req.body.local_event_id}
    })
      .then(result => {
        console.log(result);
...
module.exports.updateLocalEvent = (req, res, next) => {
...
module.exports.getLocalEvent = (req, res, next) => {
  LocalEvent.findOne({
    include: [{ model: Event, required: true }],
    ...
    .then(result => {
      if (!result) {
        return res.status(http.NOTFOUND).json("LocalEvent does not exist!");
      }
      const finalResult = Object.assign({}, {
        id: result.id,
        event_info_id: result.event_info_id,
...
module.exports.deleteLocalEvent = (req, res, next) => {
  LocalEvent.findOne({
    where: { id: req.params.id }
  })
    .then(result => {
